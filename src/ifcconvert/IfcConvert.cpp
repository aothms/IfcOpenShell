...
const std::string ELEVATION_EXTENSION = ".elevation";
const std::string GRID_EXTENSION = ".grid";
...
int main(int argc, char **argv)
{
...
#endif
			("input-file", new po::typed_value<path_t, char_t>(0), "input IFC file")("output-file", new po::typed_value<path_t, char_t>(0), "output geometry file");

	po::options_description ifc_options("IFC options");
	ifc_options.add_options()("calculate-quantities", "Calculate or fix the physical quantity definitions "
													  "based on an interpretation of the geometry when exporting IFC");

	po::options_description geom_options("Geometry options");
	geom_options.add_options()("plan",
							   ...
							   ("model",
								...
								("weld-vertices",
								 ...
								 ("use-world-coords",
								  ...
								  "case be the identity matrix.")("convert-back-units",
				...
				("include+", po::value<inclusion_traverse_filter>(&include_traverse_filter)->multitoken(),
				 ...
				 ("exclude", po::value<exclusion_filter>(&exclude_filter)->multitoken(),
				  ...
				  ("exclude+", po::value<exclusion_traverse_filter>(&exclude_traverse_filter)->multitoken(),
			       ...
			       "of the filtered entity. See --include+ for more details.")("filter-file", new po::typed_value<path_t, char_t>(&filter_filename),
																																																																																																																						 ...
																																																																																																																						 "See --include, --include+, --exclude, and --exclude+ for more details.")("no-normals",
																																																																																																																																								  ...
																																																																																																																																								  "if an object does not have any specified material in the IFC file.")("validate", "Checks whether geometrical output conforms to the included explicit quantities.");
		...
		("unicode", po::value<std::string>(&unicode_mode),
		 ...
		 "Accepted values 'utf8' (the default) and 'escape'.")
			...
			("bounds", po::value<std::string>(&bounds),
			 ...
			 "output will be scaled. Only used when converting to SVG.")("section-height", po::value<double>(&section_height),
																		 "Specifies the cut section height for SVG 2D geometry.")("use-element-names",
																																  "Use entity names instead of unique IDs for naming elements upon serialization. "
																																  "Applicable for OBJ, DAE, and SVG output.")("use-element-guids",
																																											  "Use entity GUIDs instead of unique IDs for naming elements upon serialization. "
																																											  "Applicable for OBJ, DAE, and SVG output.")("use-material-names",
																																																						  "Use material names instead of unique IDs for naming materials upon serialization. "
																																																						  "Applicable for OBJ and DAE output.")("use-element-types",
																																																																"Use element types instead of unique IDs for naming elements upon serialization. "
																																																																"Applicable for DAE output.")("use-element-hierarchy",
																																																																							  "Order the elements using their IfcBuildingStorey parent. "
																																																																							  "Applicable for DAE output.")("center-model",
																																																																															"Centers the elements upon serialization by applying the center point of "
																																																																															"all placements as an offset. Applicable for OBJ and DAE output. Can take several minutes on large models.")("model-offset", po::value<std::string>(&offset_str),
	...
	const path_t OBJ = IfcUtil::path::from_utf8(".obj"),
				 ...
				 META = IfcUtil::path::from_utf8(".meta"),
	...
	if (!attribute_filter.attribute_name.empty())
	{
		attribute_filter.update_description();
		Logger::Notice(layer_filter.description);
	}
	...
	if (output_extension == OBJ)
	{
		...
		const path_t meta_filename = change_extension(output_filename, META);
	...
	cout << "-Number of representations before: " << representations->size() << endl;
		...
		if (any_precision_encountered)
		{
			...
			{
				//kernel.setValue(IfcGeom::Kernel::GV_PRECISION, lowest_precision_encountered); // TODO: find out why this function throws segfault
			}
		}
		else
		{
	...
	// 		}
	// 	}
	// } while (++num_created, context_iterator.next(num_created));
	...
	// else
	// {
	// 	Logger::Status("\rDone creating geometry (" + boost::lexical_cast<std::string>(num_created) + " objects)                                ");
	// }
	...
	bool successful = IfcUtil::path::rename_file(IfcUtil::path::to_utf8(output_temp_filename),
